...
try:
    ...
    for ProvCode in PROVINCE_CODES_TO_RUN:
        ...
        query_data = f'''SELECT * FROM r_alldata WHERE (ProvCode = '{ProvCode}') '''
        ...
        df_all_combined.to_csv(f'log_check_dup_blue_update2_ProvCode{ProvCode}_clearDup_test_{current_datetime_str}.csv',
        ...
        log_by_case.to_csv(f'logbycase_check_dup_blue_update2_ProvCode{ProvCode}_clearDup_test_{current_datetime_str}.csv',
                        index=False, encoding='utf-8-sig')
        ...
        try:
            ...
            insert_query_r_duplicate = f"INSERT INTO r_duplicate ({', '.join(selected_columns)}) VALUES ({placeholders})"
            insert_query_r_additional = f"INSERT INTO r_additional ({', '.join(selected_columns)}) VALUES ({placeholders})"

            total_inserted_count = 0

            # --- ตรวจสอบและ Insert ลง r_duplicate ---
            print(f"\n{BLUE}กำลังตรวจสอบว่ามี ProvCode = '{ProvCode}' ในฐานข้อมูล r_duplicate หรือไม่...{RESET}")
            query_check_r_duplicate_existing = f"SELECT COUNT(*) FROM r_duplicate WHERE ProvCode = '{ProvCode}'"
            cursor.execute(query_check_r_duplicate_existing)
            r_duplicate_existing_count = cursor.fetchone()[0]

            if r_duplicate_existing_count == 0:
                print(f"{YELLOW}ไม่พบข้อมูลสำหรับ ProvCode = '{ProvCode}' ใน r_duplicate จะทำการ INSERT ข้อมูลทั้งหมด{RESET}")
                inserted_r_duplicate_count = 0
                for index, row in df_filtered.iterrows():
                    try:
                        cursor.execute(insert_query_r_duplicate, [row[col] for col in selected_columns])
                        inserted_r_duplicate_count += 1
                    except pyodbc.Error as ex:
                        sqlstate = ex.args[0]
                        print(
                            f"{RED}เกิดข้อผิดพลาดในการ INSERT ข้อมูลลง r_duplicate: {sqlstate} - {ex} - Values: {list(row)}{RESET}")
                        sql_conn.rollback()
                        raise
                print(
                    f"{PINK}Inserted {inserted_r_duplicate_count} new records for ProvCode = '{ProvCode}' into r_duplicate.{RESET}")
                total_inserted_count += inserted_r_duplicate_count
            else:
                print(
                    f"{YELLOW}พบข้อมูลสำหรับ ProvCode = '{ProvCode}' ใน r_duplicate แล้ว จะไม่ดำเนินการ INSERT เพิ่มเติม{RESET}")

            # --- ตรวจสอบและ Insert ลง r_additional ---
            print(f"\n{BLUE}กำลังตรวจสอบว่ามี ProvCode = '{ProvCode}' ในฐานข้อมูล r_additional หรือไม่...{RESET}")
            query_check_r_additional_existing = f"SELECT COUNT(*) FROM r_additional WHERE ProvCode = '{ProvCode}'"
            cursor.execute(query_check_r_additional_existing)
            r_additional_existing_count = cursor.fetchone()[0]

            if r_additional_existing_count == 0:
                print(
                    f"{YELLOW}ไม่พบข้อมูลสำหรับ ProvCode = '{ProvCode}' ใน r_additional จะทำการ INSERT ข้อมูลทั้งหมด{RESET}")
                ...
                for index, row in df_filtered.iterrows():
                    try:
                        ...
                        sqlstate = ex.args[0]
                        print(
                            f"{RED}เกิดข้อผิดพลาดในการ INSERT ข้อมูลลง r_additional: {sqlstate} - {ex} - Values: {list(row)}{RESET}")
                        sql_conn.rollback()
                        raise
                print(
                    f"{PINK}Inserted {inserted_r_additional_count} new records for ProvCode = '{ProvCode}' into r_additional.{RESET}")
                total_inserted_count += inserted_r_additional_count
            else:
                print(
                    f"{YELLOW}พบข้อมูลสำหรับ ProvCode = '{ProvCode}' ใน r_additional แล้ว จะไม่ดำเนินการ INSERT เพิ่มเติม{RESET}")

            sql_conn.commit()
            print(
                f"{GREEN}การดำเนินการ INSERT ข้อมูลสำหรับ ProvCode = '{ProvCode}' ใน r_duplicate และ r_additional เสร็จสมบูรณ์ (รวม {total_inserted_count} รายการใหม่){RESET}")
