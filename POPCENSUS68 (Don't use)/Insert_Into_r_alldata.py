...
try:
    ...
    df_building.rename(columns={'IsActive': 'Building_IsActive',
                                'Note': 'Building_Note',
                                ...
                                'UpdateDateEnd': 'Building_UpdateDateEnd'}, inplace=True)
    ...
    if 'ProvCode' in df_building.columns:
        df_building.loc[df_building['ProvCode'] == 10, 'DistName'] = df_building.loc[
            df_building['ProvCode'] == 10, 'DistName'].str.lstrip('เขต')
    else:
        print("คอลัมน์ 'ProvCode' ไม่มีอยู่ใน DataFrame.")

    # Impute RegCode And ProvCode
    df_reg_prov = pd.read_csv('C:\Users\NSO\Desktop\pop_run_data\POPCENSUS68\data\reg_prov.csv', encoding='utf-8')
    df_reg_prov['RegCode'] = df_reg_prov['RegCode'].astype(str)
    df_reg_prov['ProvCode'] = df_reg_prov['ProvCode'].astype(str)

    try:
        df_merged = pd.merge(df_building, df_reg_prov, on=['RegCode', 'ProvCode'], how='left',
                             suffixes=('_building', '_control'))

        mismatched_regname = df_merged[
            (df_merged['RegName_building'] != df_merged['RegName_control']) & df_merged['RegName_control'].notna()
            ]
        mismatched_provname = df_merged[
            (df_merged['ProvName_building'] != df_merged['ProvName_control']) & df_merged['ProvName_control'].notna()
            ]

        if not mismatched_regname.empty:
            print(f"\n{RED}--- ค่า RegName ที่ไม่ตรงกัน (df_building vs m_control_ea) ---{RESET}")
            print(mismatched_regname[['RegCode', 'ProvCode', 'RegName_building', 'RegName_control']].drop_duplicates())
        else:
        ...
        if not mismatched_provname.empty:
            ...
            print(
                mismatched_provname[['RegCode', 'ProvCode', 'ProvName_building', 'ProvName_control']].drop_duplicates())
        else:
            print(f"\n{GREEN}ไม่พบค่า ProvName ที่ไม่ตรงกัน{RESET}")

        all_mismatched_indices = mismatched_regname.index.union(mismatched_provname.index)

        if not all_mismatched_indices.empty:
            print(f"{RED}พบค่า RegName หรือ ProvName ที่ไม่ตรงกันจำนวน {len(all_mismatched_indices)} รายการ:{RESET}")
            mismatched_locations = df_merged.loc[all_mismatched_indices, ['RegCode', 'ProvCode']].drop_duplicates()

            for _, row_loc in mismatched_locations.iterrows():
                reg_code = row_loc['RegCode']
                prov_code = row_loc['ProvCode']
                correct_data = df_reg_prov[
                    (df_reg_prov['RegCode'] == reg_code) & (df_reg_prov['ProvCode'] == prov_code)]
                ...
                if not correct_data.empty:
                    ...
                    rows_to_update = df_building[
                        (df_building['RegCode'] == reg_code) & (df_building['ProvCode'] == prov_code)].index
                    if not rows_to_update.empty:
                        df_building.loc[rows_to_update, 'RegName'] = correct_regname
                        df_building.loc[rows_to_update, 'ProvName'] = correct_provname
                        print(
    ...
    # เปลี่ยนชื่อคอลัมน์ 'IsActive' เป็น 'IsActive_Buil'
    df_household.rename(columns={'Building_No': 'Building_No_HH',
                                 ...
                                 'UpdateDateEnd': 'Household_UpdateDateEnd'}, inplace=True)

    # Merge df_building กับ df_household
    df_merged_bh = pd.merge(df_building, df_household, left_on='Building_No', right_on='Building_No_HH', how='left')
    ...
    # เปลี่ยนชื่อคอลัมน์
    df_population.rename(columns={'Building_No': 'Building_No_POP',
                                 ...
                                 'UpdateDateEnd': 'Population_UpdateDateEnd',
                                  'Longitude': 'Longitude_POP'}, inplace=True)

    str(df_population['Age_01']).zfill(3)

    # Merge df_merged_bh กับ df_population
    df_merged_bhp = pd.merge(df_merged_bh, df_population, left_on=['Building_No', 'Household_No'],
                               right_on=['Building_No_POP', 'Household_No_POP'], how='left')
    df_merged_bhp = df_merged_bhp.fillna('')
    print("Shape of df_merged_bhp_merged:", df_merged_bhp.shape)
    print("คอลัมน์ที่ซ้ำกันใน df_merged_bhp_merged:", df_merged_bhp.columns[df_merged_bhp.columns.duplicated()])
    ...
    df_m_control_ea.rename(columns={'CrtdByCode': 'm_control_ea_CrtdByCode',
                                    'AreaCode': 'AreaCode_m',
                                    'VillCode': 'VilCode_m',
                                    'VillName': 'VilName_m',
    ...
    df_final_merged = pd.merge(df_merged_bhp, df_m_control_ea, left_on=['EA_Code_15'],
                               right_on=['EA_Code_15_m'], how='left')
    ...
    """
    # กรองด้วย m_control_ea
    query_m_control_ea = '''SELECT RegCode, ProvCode, DistCode, SubDistCode, AreaCode, EA_NO FROM m_control_ea WHERE ADV_EA = 'A' '''
    df_m_control_ea = pd.read_sql(query_m_control_ea, sql_conn).fillna('')
    df_m_control_ea = df_m_control_ea.rename(columns={'EA_NO': 'EA_No'})
    df_final_merged = pd.merge(df_final_merged, df_m_control_ea,
                               on=['RegCode', 'ProvCode', 'DistCode', 'SubDistCode', 'AreaCode', 'EA_No'], how='inner')
    print(f"Shape of df_final_merged With m_control_ea = A: {df_final_merged.shape}")
    """


    # กรองขั้นสุดท้าย
    df_final_merged = df_final_merged[(df_final_merged['ApproveStatusProv'] == 'Y')].copy()
    ...
    def generate_all_unique_random_suffixes(num_records, num_digits=8):
        ...
        return [str(i).zfill(num_digits) for i in np.random.choice(max_num_possible, num_records, replace=False)]
    ...
    if total_records > max_num_possible_suffixes:
        print(f"{RED}**ข้อผิดพลาดร้ายแรง: จำนวนเรคคอร์ดทั้งหมด ({total_records}) เกินกว่าจำนวนตัวเลขสุ่ม 8 หลัก ({max_num_possible_suffixes}). "
    ...
    random_suffixes = generate_all_unique_random_suffixes(total_records, num_digits=8)
    ...
    df_final_merged['IDEN'] = df_final_merged['IDEN_prefix'] + df_final_merged['IDEN_suffix']
    ...
    print("มีค่า IDEN ซ้ำกันหรือไม่ (หลังการสร้างและก่อน Export):", df_final_merged['IDEN'].duplicated().any())
    if df_final_merged['IDEN'].duplicated().any():
        ...
        print(df_final_merged[df_final_merged['IDEN'].duplicated(keep=False)].sort_values('IDEN').head())

        # เพิ่มการตรวจสอบเพิ่มเติมหาก IDEN ยังซ้ำ (อาจเกิดจากข้อมูลต้นทาง หรือ EA_Code_15 ซ้ำกันอย่างมีนัยสำคัญ)
        duplicated_idens = df_final_merged[df_final_merged['IDEN'].duplicated(keep=False)]
        if not duplicated_idens.empty:
            print(f"{RED}**พบปัญหา: IDEN เดียวกันถูกใช้โดย Population_No ที่แตกต่างกัน หรือมี Population_No เป็นค่าว่าง หรือ EA_Code_15 ที่ซ้ำกันทำให้ IDEN ซ้ำ!**{RESET}")
            print(duplicated_idens[['IDEN', 'Population_No', 'EA_Code_15']].sort_values('IDEN'))
    else:
    ...
    vil_mapping = df_m_control_ea.set_index('EA_Code_15_m')[['VilCode_m', 'VilName_m']]
    ...
    condition = (df_final_merged['VilCode'].isna()) | (df_final_merged['VilCode'].str.len() < 2)

    # เติมค่า VilCode_m และ VilName_m เฉพาะแถวที่ตรงตามเงื่อนไข
    df_final_merged.loc[condition, 'VilCode'] = df_final_merged.loc[condition, 'EA_Code_15'].map(
        vil_mapping['VilCode_m'])
    df_final_merged.loc[condition, 'VilName'] = df_final_merged.loc[condition, 'EA_Code_15'].map(
        vil_mapping['VilName_m'])
    ...
    df_final_merged['Household_CrtdByName'] = df_final_merged['Household_CrtdByName'].str.strip()
    ...
    is_peoples = df_final_merged['Household_CrtdByName'].str.contains('ประชาชน', na=False)
    ...
    selected_columns = [
        'IDEN', 'EA_Code_15', 'Building_No', 'BuildingCode', 'NsoBuilding_No', 'RegCode', 'RegName', 'ProvCode',
                        ...
                        'Population_IsActive', 'Population_Note', 'Longitude_POP','Population_CrtdByCode', 'Population_CrtdByName', 'Population_CrtdDateStart',
                        'Population_CrtdDateEnd', 'Population_UpdateDateStart', 'Population_UpdateDateEnd', 'ApproveStatusSup', 'ApproveDateSup',
    ...
    #print('เลือกคอลัมน์ เสร็จแล้ว ถัดไป เขียนไฟลื CSV')

    #output_path2 = fr'D:\Pop_Census67_68\Pop68\Data_For_check2\Data_Approve-Y_{ProvCode}_{current_datetime_str}.csv'
    #df_filtered.to_csv(output_path2, index=False, encoding='utf-8-sig')
    #print(f"ข้อมูลถูก Export ไปที่: {output_path2} เรียบร้อยแล้ว")

    # --- ซิงค์ข้อมูลกับฐานข้อมูล r_alldata และ r_alldata_edit (เฉพาะการเพิ่มข้อมูล) ---
    ...
    try:
        ...
        query_check_r_alldata_existing = f"SELECT COUNT(*) FROM r_alldata WHERE ProvCode = '{ProvCode}'"
        cursor.execute(query_check_r_alldata_existing)
        r_alldata_existing_count = cursor.fetchone()[0]

        if r_alldata_existing_count == 0:
            print(f"{YELLOW}ไม่พบข้อมูลสำหรับ ProvCode = '{ProvCode}' ใน r_alldata จะทำการ INSERT ข้อมูลทั้งหมด{RESET}")
            inserted_r_alldata_count = 0
            for index, row in df_filtered.iterrows():
                try:
                    cursor.execute(insert_query_r_alldata, [row[col] for col in selected_columns])
        ...
        """
        # --- ตรวจสอบและ Insert ลง r_alldata_edit ---
        print(f"\n{BLUE}กำลังตรวจสอบว่ามี ProvCode = '{ProvCode}' ในฐานข้อมูล r_alldata_edit หรือไม่...{RESET}")
        query_check_r_alldata_edit_existing = f"SELECT COUNT(*) FROM r_alldata_edit WHERE ProvCode = '{ProvCode}'"
        cursor.execute(query_check_r_alldata_edit_existing)
        r_alldata_edit_existing_count = cursor.fetchone()[0]

        if r_alldata_edit_existing_count == 0:
            print(
                f"{YELLOW}ไม่พบข้อมูลสำหรับ ProvCode = '{ProvCode}' ใน r_alldata_edit จะทำการ INSERT ข้อมูลทั้งหมด{RESET}")
            inserted_r_alldata_edit_count = 0
            for index, row in df_filtered.iterrows():
                try:
                    cursor.execute(insert_query_r_alldata_edit, [row[col] for col in selected_columns])
                    inserted_r_alldata_edit_count += 1
                except pyodbc.Error as ex:
                    sqlstate = ex.args[0]
                    print(
                        f"{RED}เกิดข้อผิดพลาดในการ INSERT ข้อมูลลง r_alldata_edit: {sqlstate} - {ex} - Values: {list(row)}{RESET}")
                    sql_conn.rollback()
                    raise
            print(
                f"{PINK}Inserted {inserted_r_alldata_edit_count} new records for ProvCode = '{ProvCode}' into r_alldata_edit.{RESET}")
            total_inserted_count += inserted_r_alldata_edit_count
        else:
            print(
                f"{YELLOW}พบข้อมูลสำหรับ ProvCode = '{ProvCode}' ใน r_alldata_edit แล้ว จะไม่ดำเนินการ INSERT เพิ่มเติม{RESET}")
        """
        sql_conn.commit()
        print(
